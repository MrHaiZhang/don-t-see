...
var emotions_temp = '<div id="J_emotions_pop" style="z-index:11;position:absolute;" class="core_menu pop_show_mini">\
						<div class="core_arrow_top" style="left:0;"><em></em><span></span></div><a href="javascript:void(0);" id="" onclick="J_emotions_close()" class="pop_close">关闭</a>\
...
function insertEmotions(elem, input, wrap) {
				...
				if(data.state == 'success') {
					...
					$.each(data.data,function(i, o){
						...
						$.each(o.emotion, function(i, o){
							emotion_arr.push('<li><a href="#" class="J_emotions_item" data-sign="'+ o.sign +'" data-src="'+o.url+'"><img '+ (index === 1 ? 'src=\"'+o.url+'\"' : 'data-src=\"'+o.url+'\"') +'></a></li>');
						});
	...
	$('#J_emotions_pl').off('click').on('click', 'a.J_emotions_item', function(e){
		...
		if(glo_text_node){
			...
			//el.setAttribute('style', 'background: url(\''+this.getAttribute('data-src')+'\') 0% 0% / cover no-repeat');
	        var el = document.createElement('img');
	        el.src = this.getAttribute('data-src');
	        el.className = 'emoji_pic';
			insertTextAtCursor(el, input);
		} else{
			//input.append("<hr class='emoji_pic' style='background: url("+this.getAttribute('data-src')+") no-repeat; background-size: cover;' />")
			input.append("<img class='emoji_pic' src='"+this.getAttribute('data-src')+"' />")
		}
...
function emotionsShowPage(index, i){
	...
	for(var i = (index-1)*page_size; i <= len - 1; i++) {
		arr.push('<li><a href="#" class="J_emotions_item" data-sign="'+ data[i].sign +'" data-src="'+data[i].url+'"><img src="'+data[i].url+'"></a></li>');
	}
