...
class ManageController extends AdminBaseController {
	...
	public function run() {
		...
		ksort($groups);
		list($sName, $sensitive, $hasSensitive, $sUid, $sEmail, $sGroup, $page) = $this->getInput(array('username', 'sensitive', 'hasSensitive', 'uid', 'email', 'gid', 'page'));
		$vo = new PwUserSo();
		$sName && $vo->setUsername($sName);
		$sensitive && $vo->setSensitive($sensitive);
		$hasSensitive && $vo->setHasSensitive($hasSensitive);
		$sUid && $vo->setUid($sUid);
		...
		if (0 < $count) {
			...
			if ($result) {
				...
				$list = $userDs->fetchUserByUid(array_keys($result), PwUser::FETCH_ALL);
	...
	public function doEditAction() {
		...
		$dm->setNickname($this->getInput('nickname', 'post'));
	...
	public function doEditCreditAction() {
		...
		$creditBo->addLog('admin_set', $changes, new PwUserBo($info['uid']));
