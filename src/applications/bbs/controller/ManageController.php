...
class ManageController extends PwBaseController {
	...
	protected function _getCombinedManage($manage) {
		...
		if (!$this->doAction) {
			...
			//公告默认全局置顶
			if(in_array('notice', $actions) && !in_array('topped', $actions)){
				$actions[] = 'topped';
			}
	...
	//官方标签
	protected function _getOfficialManage($manage) {
		Wind::import('SRV:forum.srv.manage.PwThreadManageDoOfficial');
		$do = new PwThreadManageDoOfficial($manage);
		if (!$this->doAction) {

		} else {
			$official = $this->getInput('official', 'post');
			$do->setOfficial($official);
			$do->setReason($this->getInput('reason', 'post'));
			!$official && $this->doCancel[] = 'doofficial';
		}
		return $do;
	}

	//公告标签
	protected function _getNoticeManage($manage) {
		Wind::import('SRV:forum.srv.manage.PwThreadManageDoNotice');
		$do = new PwThreadManageDoNotice($manage);
		if (!$this->doAction) {

		} else {
			$notice = $this->getInput('notice', 'post');
			$do->setNotice($notice);
			$do->setReason($this->getInput('reason', 'post'));
			!$notice && $this->doCancel[] = 'donotice';
		}
		return $do;
	}
	...
	protected function _getToppedManage($manage) {
		...
		if (!$this->doAction) {
			...
			$actions = $this->getInput('actions', 'post');
			if(in_array('notice', $actions) && !in_array('topped', $actions)){
				if($this->getInput('notice', 'post') == 1){
					$topped = '3';
					$toppedOvertime = '2037-01-01';
				} else{
					$topped = '0';
					$toppedOvertime = '';
				}
