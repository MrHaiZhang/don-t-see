...
class SetforumController extends AdminBaseController {
	...
	public function dorunAction() {
		...
		//设置专区信息
		$list = $pwForum->getCommonForumList(PwForum::FETCH_MAIN);
		$forumData = array();
		...
		$redis_obj->setForumData($forumData);
	...
	public function editAction() {
		...
		$forum->foruminfo['get_url'] = $forum->foruminfo['get_url'] ? json_decode($forum->foruminfo['get_url'], true) : array();
	...
	private function _updateForums($forum, $copyFids = array(), $copyItems = array()) {
		...
		list($forumname, $vieworder, $parentid, $descrip, $isshow, $isshowsub, $jumpurl, $seotitle, $seokeywords, $seodescription, $numofthreadtitle, $threadperpage, $readperpage, $newtime, $threadorderby, $minlengthofcontent, $locktime, $edittime, $allowtype, $typeorder, $contentcheck, $ifthumb, $thumbwidth, $thumbheight, $anticopy, $copycontent, $water, $waterimg, $allowhide, $allowsell, $anonymous, $manager, $creditset, $password, $allowvisit, $allowread, $allowpost, $allowreply, $allowupload, $allowdownload, $style, $get_url, $game_id, $sdk_game_id) = $this->getInput(array('forumname', 'vieworder', 'parentid', 'descrip', 'isshow', 'isshowsub', 'jumpurl', 'seotitle', 'seokeywords', 'seodescription', 'numofthreadtitle', 'threadperpage', 'readperpage', 'newtime', 'threadorderby', 'minlengthofcontent', 'locktime', 'edittime', 'allowtype', 'typeorder', 'contentcheck', 'ifthumb', 'thumbwidth', 'thumbheight', 'anticopy', 'copycontent', 'water', 'waterimg', 'allowhide', 'allowsell', 'anonymous', 'manager', 'creditset', 'password', 'allowvisit', 'allowread', 'allowpost', 'allowreply', 'allowupload', 'allowdownload', 'style', 'get_url', 'game_id', 'sdk_game_id'));

		//链接增加默认值
		$config = Wekit::C()->getValues('site');
		!$get_url[0] && $get_url[0] = $config['info.mobweb'].'/index.php';
		!$get_url[1] && $get_url[1] = $config['info.url'].'/index.php?m=wap&c=Gift&fid='.$mainFid;
		!$get_url[2] && $get_url[2] = $config['info.mobweb'].'/index.php?c=news';
		!$get_url[3] && $get_url[3] = $config['info.mobweb'].'/index.php?c=customer';
		$get_url = json_encode($get_url);
		Wind::import('SRV:forum.bo.PwForumBo');
		Wind::import('SRV:forum.dm.PwForumDm');
		$pwforum = Wekit::load('forum.PwForum');
