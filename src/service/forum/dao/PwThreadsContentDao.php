...
class PwThreadsContentDao extends PwBaseDao {
	...
	public function fetchThreadByTid($tids, $limit, $offset) {
		if (!$result = $this->getBaseInstance()->fetchThreadByTid($tids, $limit, $offset)) {
			return $result;
		}
		return $this->_margeArray($result, $this->_fetch(array_keys($result), 'tid'));
	}
	...
	public function getThreadByFidHot($fid, $limit, $offset) {
		if (!$result = $this->getBaseInstance()->getThreadByFidHot($fid, $limit, $offset)) {
			return $result;
		}
		return $this->_margeArray($result, $this->_fetch(array_keys($result), 'tid'));
	}
	...
	public function getThreadByUidSuppress($uid) {
		if (!$result = $this->getBaseInstance()->getThreadByUidSuppress($uid)) {
			return $result;
		}
		return $this->_margeArray($result, $this->_fetch(array_keys($result), 'tid'));
	}
