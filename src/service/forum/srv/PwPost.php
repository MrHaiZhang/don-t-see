...
class PwPost extends PwBaseHookService {
	...
	//增加被回复人id的设置
	public function setBeReplyUid($bereplyuid) {
		$this->bereplyuid = intval($bereplyuid);
	}
	...
	public function execute(PwPostDm $postDm) {
		//增加敏感词记录
		$result_obj = $this->action->beforeRun($postDm);
		if ($result_obj instanceof PwError) {
			return $result_obj;
		} else if($result_obj instanceof PwPostDm){
			$this->action->setDm($result_obj);
			$postDm = $result_obj;
		}
