...
class PwReplyPost extends PwPostAction {
	...
	protected $pid;
	protected $info;
	private $checkContentHash;

	public function __construct($tid, PwUserBo $user = null, $checkContentHash = true) {
		$this->tid = $tid;
	...
	public function dataProcessing(PwPostDm $postDm) {
		...
		if ($checkContentHash && ($result = $this->checkContentHash($postDm->getContent())) !== true) {
			return $result;
		}
	...
	public function afterRun() {
		//回复提醒规则有问题，自己组装参数发
		$post_data = $this->postDm->get_data();
		
		//二次回复
		if($rpid = $post_data['rpid']){
			if($post_data['replyid']){
				$rpid = $post_data['replyid'];
			}
			//获取二次回复的信息
			$reply_data = Wekit::load('forum.PwThread')->getPost($rpid);
			$params = array(
				'replyUserid'=>$post_data['created_userid'],
				'replyUsername'=>$post_data['created_username'],
				'postTitle'=>$this->info['subject'],
				'threadId'=>$this->info['tid'],
				'pid'=>$this->pid,
				'rpid'=>$rpid,
				'postUserid'=>$reply_data['created_userid'],
				);
			$noticeService = Wekit::load('message.srv.PwNoticeService');
			$noticeService->sendNotice($reply_data['created_userid'], 'postreply', $rpid, $params);
			//去除提醒
			$this->runDo('addPost', 0, 0);
		} else{		//一次回复
