...
class PwCommonRead extends PwReadDataSource {
	...
	public function execute() {
		...
		if ($this->info['replies'] > 0) {
			...
			if ($limit > 0) {
				//获取一级回复
				$replies = array_merge($replies, $this->thread->getRepliesGroup($limit, $offset, $this->asc));
			}
			foreach ($replies as $value) {
				//下面有回复时
				if($value['replies'] > 0){
					//获取二级回复，降序排序,默认5条
					$value['replies_list'] = array_merge($this->thread->getRepliesGroup(5, 0, false, $value['pid']));
				}
				$this->data[] = $value;
