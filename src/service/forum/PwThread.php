...
class PwThread {
	 ...
	 * @return array
	 */
	public function getThreadByFidHot($fid, $limit, $offset = 0, $fetchmode = self::FETCH_MAIN) {
		return $this->_getThreadDao($fetchmode)->getThreadByFidHot($fid, $limit, $offset);
	}
	 ...
	 * @return array
	 */
	public function getThreadByFidHotCount($fid, $fetchmode = self::FETCH_MAIN) {
		return $this->_getThreadDao($fetchmode)->getThreadByFidHotCount($fid);
	}

	//查询公告列表总数
	public function getThreadByFidNotice($fid) {
		return $this->_getThreadDao()->getThreadByFidNotice($fid);
	}

	public function fetchThreadByTid($tids, $limit, $start, $fetchmode = 7) {
	 ...
	 * @param bool $asc
	 * @param int $rpid
	 * @param bool $hot 	//回复数排序
	 * @param int $pid
	 * @return array
	 */
	public function getPostByTidGroup($tid, $limit = 20, $offset = 0, $asc = true, $rpid = 0, $hot = false, $pid = 0) {
		if (empty($tid)) return array();
		return $this->_getPostDao()->getPostByTidGroup($tid, $limit, $offset, $asc, $rpid, $hot, $pid);
	}
	...
	public function getPostByTidGroupCount($tid) {
		if (empty($tid)) return array();
		return $this->_getPostDao()->getPostByTidGroupCount($tid);
	}
	...
	public function getPostByTidGroupLou($tid, $pid) {
		if (empty($tid)) return array();
		return $this->_getPostDao()->getPostByTidGroupLou($tid, $pid);
	}
	 ...
	 * @param int $end_time
	 * @param int $disabled 显示状态
	 * @param int $reply 是否有回复id(有则是楼层中的回复)
	 * @return int
	 */
	public function countPostByUidAndList($uid, $start_time = false, $end_time = false, $disabled = false, $reply = false) {
		if (empty($uid)) return 0;
		return $this->_getPostDao()->countPostByUidAndList($uid, $start_time, $end_time, $disabled, $reply);
	}
