...
class PwMessageNotices {
	...
	public function getTypeUnreadNoticeCount($uid, $type = ''){
		return $this->_getDao()->getTypeUnreadNoticeCount($uid, $type);
	}

	/**
	 * 获取七天内有登录且有未读通知的用户
	 */
	public function getAllUnreadNoticeCount($offset, $num){
		return $this->_getDao()->getAllUnreadNoticeCount($offset, $num);
	}
