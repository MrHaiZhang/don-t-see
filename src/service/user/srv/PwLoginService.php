...
class PwLoginService extends PwBaseHookService {
	 ...
	 * @param bool $checkOff 是否验证
	 * @return boolean|int
	 */
	public function login($username, $password, $ip, $safeQuestion = null, $safeAnswer = '', $checkOff = true) {
		if($retrun_arr = $this->checkBanLogin($username)){
			return $retrun_arr;
		}
		...
		$info = $pwdBp->auth($username, $password, $ip, $checkQ, $safeQuestion, $safeAnswer, $checkOff);
	...
	/**
	 * [checkBanLogin 查找是否禁止登陆]
	 * @param  [type] $username [用户名]
	 * @return [type]           [description]
	 */
	public function checkBanLogin($username){
		Wind::import('SRV:user.dao.PwUserDao');
		$userDao = new PwUserDao();
		$user_info = $userDao->getUserByName($username);
		if($user_info['uid']){
			Wind::import('SRV:user.vo.PwUserBanSo');
			$page = 1;
			$perpage = 1;
			$searchSo = new PwUserBanSo();
			$searchSo->setType('uid')
			    ->setTypeId(5)
				->setKeywords($user_info['uid']);
			/* @var $banDs PwUserBan */
			$banDs = Wekit::load('user.PwUserBan');
			$count = $banDs->countWithCondition($searchSo);

			if($count >= 1){
				return array('code'=>'-1', 'msg'=>'banLogin');
			}
		}
		return false;
	}
