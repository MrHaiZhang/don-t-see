...
class PwUser {
	...
	public function getPunchNum($limit) {
		...
		return $this->_getDao(PwUser::FETCH_DATA)->getPunchNum($limit);
	}

	//获取固定打卡次数的用户
	public function getPunchSpNum($num) {
		if ($num <= 0) return array();
		return $this->_getDao(PwUser::FETCH_DATA)->getPunchSpNum($num);
	}

	//获取积分区间内的用户总数
	public function getCreditCount($start, $end, $credit_num = 1) {
		if ($end <= 0) return 0;
		return $this->_getDao(PwUser::FETCH_DATA)->getCreditCount($start, $end, $credit_num);
	}

	//获取最多积分的用户数据
	public function getCreditMax($credit_num = 1) {
		return $this->_getDao(PwUser::FETCH_DATA)->getCreditMax($credit_num);
	}
