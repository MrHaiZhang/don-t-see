 ...
 *
 * 增加是否是官网用户的字段off_user,唯一十位数字字段ident_id,获取ident_id
 */
class PwUserDao extends PwBaseDao {

	protected $_table = 'user';
	protected $_pk = 'uid';
	protected $_dataStruct = array('uid', 'username', 'email', 'password', 'regdate', 'realname', 'status', 'groupid', 'memberid', 'groups' ,'off_user', 'ident_id');
	 ...
	 * @return bool
	 */
	public function getIdentId($ident_id) {
		$sql = $this->_bindTable('SELECT * FROM %s WHERE ident_id=?');
		$smt = $this->getConnection()->createStatement($sql);
		return $smt->getOne(array($ident_id));
	}
