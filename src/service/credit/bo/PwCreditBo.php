...
class PwCreditBo {
	...
	public function addLog($operation, $setv, PwUserBo $user, $log = array()) {
		...
		$log['uid'] = $user->uid;
		//特殊处理
		if($operation == 'admin_set'){
			if($log['action_reason']){
				$log['username'] = $log['adminname'].'-'.$log['action_reason'];
			} else{
				$log['username'] = $log['adminname'].'-重置用户积分';
			}
		...
		foreach ($setv as $key => $affect) {
			if (isset($this->cType[$key]) && $this->isLog[$key] && $affect <> 0 && $key == 1) {
				$log['cname'] = $this->cType[$key];
				$log['affect'] = $affect > 0 ? '+' . $affect : $affect;
				$descrip = $coc->getDescrip($operation, $log);
				if(!$this->synHomeCredit($user->username, $setv, $operation, '【论坛】'.$descrip)){
					return false;
				}
			}
		}
	...
	/**
	 * [synHomeCredit 向官网同步积分]
	 * @param  [type] $username  [用户名]
	 * @param  [type] $arr       [积分数组]
	 * @param  [type] $operation [操作类型]
	 * @param  [type] $desc      [操作详情]
	 * @return [type]            [description]
	 */
	public function synHomeCredit($username, $arr, $operation, $desc){
		//屏蔽积分1外的其他积分
		if(!$arr[1]){
			return false;
		}
