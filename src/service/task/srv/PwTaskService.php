...
class PwTaskService {
	...
	public function getAutoApplicableTaskList($uid, $limit = 1) {
		...
		//获取所有自动申领任务
		$ids = $this->_taskDs()->getAutoApplicableTask(0, $gids, abs(intval($limit)));
	 ...
	 *
	 * 每个任务有三个状态信息：
	 * 1-tag: 右上角显示:1:已领取，2：已关闭，3：已结束，4：正在进行中，5: 领取奖励--已完成，6: 已完成，
	 * 2-continue: 按钮是否可以使用
	 * 3-percent: 任务进行的进度
	 * 
	...
	private function _buildMyTaskList($taskList, $myTask) {
		...
		foreach ($taskList as $id => &$task) {
			...
			//任务完成次数
			$task['ed_num'] = is_array($step) && isset($step['end_num']) ? $step['end_num'] : 0;
			//任务完成时间
			$task['finish_time'] = $myTask[$id]['finish_time'];
			/*已有进度并且状态为已领取的变为正在进行中*/
			if ($task['percent'] && $task['tag'] == 1) $task['tag'] = 4;
			if ($task['percent'] == '100%') $task['tag'] = 6;
		}
		return $taskList;
	}
